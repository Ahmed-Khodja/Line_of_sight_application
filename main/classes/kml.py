...
class Kml:
    ...
    def create_points(self):
        for i in self.loc1:
            point = self.fol1.newpoint(name=i.name, coords=[(i.x, i.y, float(i.height)*0.3048)])
            point.altitudemode = simplekml.AltitudeMode.relativetoground
            point.extrude = 1

        for i in self.loc2:
            point = self.fol2.newpoint(name=i.name, coords=[(i.x, i.y,float(i.height)*0.3048)])
            point.altitudemode = simplekml.AltitudeMode.relativetoground
            point.extrude = 1
    ...
    def create_linestrings(self):
        ...
        for i in locs1:
            for j in locs2:
                ...
                linestring.coords = [(i.x, i.y, float(i.height)*0.3048), (j.x, j.y, float(j.height)*0.3048)]
