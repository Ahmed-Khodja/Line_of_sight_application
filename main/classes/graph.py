...
class Graph:
    ...
    def build(self):
        try:
            ...
            plt.title("CY_ENNA_TOOL -Visibilité Avion/ VHF-")
            plt.xlabel("Distance (" + self.loc1.distance_units + ")"),
            plt.ylabel("Elevation (" + self.loc1.height_units + ")"),
            plt.style.use('seaborn-dark')
            plt.grid()

            f = open("C:\CY_ENNA_Tool\OUTPUT\DataForMatlab.csv", "w")
            s = self.check_intersect()
            print(s)
            y = 120
    ...
    def get_los_line(self):
        print("Elevation data:", self.elevation_data)  # Add this line to check the data
    ...
    def check_intersect(self):
        ...
        while i < len(self.los_line): 
            if self.los_line[i] < self.final_elevation[i]:
                # print(str(self.los_line[i]) + " !! "  + str(self.elevation_data[i]))
                print(str(self.los_line[i]) + " !! "  + str(self.final_elevation[i]))
                return True
            else:
